...
import { provisionDomain, provisionSubdomain } from './provisioning.js';
...
const app = express();
app.disable('x-powered-by'); app.use(helmet({ contentSecurityPolicy: false })); app.use(express.json({ limit: '1mb' })); app.use(cookieParser());
app.get('/api/health', (_req, res) => res.json({ success: true, service: 'hostingpenal-api', time: new Date().toISOString() }));
app.post('/api/auth/login', async (req, res) => { try { const { username, password } = req.body ?? {}; const valid = await verifyMasterCredentials(String(username || ''), String(password || '')); if (!valid) return res.status(401).json({ success: false, message: 'Invalid username or password' }); const token = createSession(config.masterUsername); res.cookie(config.cookieName, token, cookieOptions()); return res.json({ success: true, user: { username: config.masterUsername, role: 'Super Administrator' } }); } catch (error) { console.error('login error', error); return res.status(500).json({ success: false, message: 'Authentication service unavailable' }); } });
app.post('/api/auth/logout', (req, res) => { destroySession(req.cookies?.[config.cookieName]); res.clearCookie(config.cookieName, { httpOnly: true, secure: config.nodeEnv === 'production', sameSite: 'strict', path: '/' }); res.json({ success: true }); });
...
app.get('/api/dns/resolve', requireAuth, async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      domain,
      type,
      answers: stdout.trim() ? stdout.trim().split('\n') : [],
    });
  } catch {
    res.status(502).json({
      success: false,
      message: 'DNS resolver query failed',
    });
  }
});
...
app.post('/api/domains/provision', requireAuth, async (req, res) => {
  try {
    ...
    const result = await provisionDomain({
      ...
      username: String(username || config.hostingUsername).toLowerCase(),
      phpSocket,
      serverIp: String(
        serverIp || config.serverPublicIp || ''
      ).trim(),
    ...
    res.status(400).json({
      success: false,
      message: error?.message || 'Domain provisioning failed',
    });
  }
});

app.post('/api/domains/subdomain/provision', requireAuth, async (req, res) => {
  try {
    const {
      subdomain,
      parentDomain,
    ...
    const result = await provisionSubdomain({
      subdomain,
      parentDomain: String(parentDomain || '').toLowerCase(),
      username: String(username || config.hostingUsername).toLowerCase(),
      phpSocket,
      serverIp: String(
        serverIp || config.serverPublicIp || ''
    ...
    console.error('subdomain provision error', error);
    res.status(400).json({
      success: false,
      message: error?.message || 'Subdomain provisioning failed',
    });
  }
});
...
app.get('/api/integrations/status', requireAuth, async (_req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: error?.message || 'Integration status failed',
    });
  }
});
...
app.get('/api/integrations/:name/install-plan', requireAuth, (req, res) => {
  try {
    res.json({
      success: true,
      ...getInstallPlan(req.params.name),
    });
  } catch (error) {
    res.status(400).json({
      success: false,
      message: error?.message || 'Unsupported integration',
    });
  }
});
...
app.post('/api/integrations/:name/install', requireAuth, async (req, res) => {
  try {
    ...
    if (
      execute &&
      req.user.username !== config.masterUsername
    ) {
    ...
    res.status(400).json({
      success: false,
      message: error?.message || 'Integration installation failed',
    });
  }
});
...
app.listen(config.port, config.host, () => {
  console.log(
    `HostAdmin API listening on http://${config.host}:${config.port}`
  );
});