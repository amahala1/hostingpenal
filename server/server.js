...
app.post('/api/domains/provision', requireAuth, async (req, res) => {
  try {
    const { domain, username, phpSocket, serverIp, issueSsl = false } = req.body ?? {};
    const result = await provisionDomain({
      domain: String(domain || '').toLowerCase(),
      username: String(username || config.hostingUsername).toLowerCase(),
      phpSocket,
      serverIp: serverIp || config.serverPublicIp,
...
app.post('/api/domains/subdomain/provision', requireAuth, async (req, res) => {
  try {
    const { subdomain, parentDomain, username, phpSocket, serverIp, dnsType = 'A', dnsTarget, issueSsl = false } = req.body ?? {};
    ...
    res.status(400).json({ success: false, message: error?.message || 'Subdomain provisioning failed' });
  }
});
